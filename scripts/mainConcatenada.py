...
#====================================================================================================================================================================================================
# MODELOS DE DOMÍNIO (DATA OBJECTS)
#====================================================================================================================================================================================================
@dataclass
class Instruction:
    """
...
# =========================================================================================
# 2. CAMADA DE INTEGRAÇÃO (API CLIENT)
# =========================================================================================

class GlpiClient:
    """
    API Wrapper / Cliente HTTP Centralizado:
    Implementa o padrão de projeto 'Singleton/Facade' para as comunicações de rede.
    Gerencia a sessão ativa, os tokens de autorização e a supressão de verificação SSL (Intranet),
    evitando redundância de código (DRY - Don't Repeat Yourself) nas chamadas REST.
    """
    def __init__(self, api_url: str, app_token: str, session_token: str):
        self.base_url = api_url.rstrip("/")
        
        # Headers padronizados para tráfego JSON (Consultas e Atualizações)
        self.headers_json = {
            "App-Token": app_token,
        ...
        # Headers padronizados para tráfego Multipart (Upload de evidências/Arquivos)
        self.headers_multipart = {
            "App-Token": app_token,
            "Session-Token": session_token,
        }
        
        # Connection Pooling: Mantém uma sessão HTTP aberta para ganho de performance
        self.client = httpx.Client(
            base_url=self.base_url,
            headers=self.headers_json,
            verify=False,
            timeout=15.0 # Previne gargalos de rede (Thread hanging)
        )

    def get(self, endpoint: str, params: dict = None):
        """Método encapsulado para requisições de leitura (GET)."""
        return self.client.get(endpoint, params=params)

    def put(self, endpoint: str, payload: dict):
        """Método encapsulado para requisições de atualização (PUT)."""
        return self.client.put(endpoint, json={"input": payload})
        
    def post(self, endpoint: str, payload: dict):
        """Método encapsulado para requisições de criação (POST)."""
        return self.client.post(endpoint, json={"input": payload})
        
    def delete(self, endpoint: str):
        """Método encapsulado para requisições de deleção (DELETE)."""
        return self.client.delete(endpoint)
        
    def upload_file(self, endpoint: str, files: dict):
        """Trata especificamente o envio de dados binários (Anexos HTML/CSV)."""
        return httpx.post(
...
#====================================================================================================================================================================================================
# SENDER - from src.data.sender
#====================================================================================================================================================================================================
class GlpiClient:
    """
    Client centralizado para comunicação com a API do GLPI.
    Gerencia a sessão HTTP, cabeçalhos de autenticação e supressão de SSL, evitando redundância de código (DRY) nas chamadas de rede.
    """
    def __init__(self, api_url: str, app_token: str, session_token: str):
        self.base_url = api_url.rstrip("/")
        # Headers padronizados para tráfego JSON (Consultas e Atualizações)
        ...
        # Cria uma sessão persistente (melhora performance e gestão de memória)
        self.client = httpx.Client(base_url = self.base_url, headers = self.headers, verify = False, timeout = 15.0)

    def get(self, endpoint: str, params: dict = None): return self.client.get(endpoint, params = params)

    def post(self, endpoint: str, payload: dict): return self.client.post(endpoint, json = {"input": payload})

    def put(self, endpoint: str, payload: dict): return self.client.put(endpoint, json = {"input": payload})
        
    def delete(self, endpoint: str): return self.client.delete(endpoint)

    def upload_file(self, endpoint: str, files: dict): return httpx.post(f"{self.base_url}{endpoint}", headers = self.headers_multipart, files = files, verify = False)
...
def gerarHistoricoCsv(results: List[Result], chamadoId: str, tarefaId: str) -> str:
    ...
    with open(filePath, "w", newline="", encoding="utf-8") as csvFileObj:
        ...
        for result in results:
            ...
            # Deserialização da string estruturada de status para colunas do CSV
            ...
            writer.writerow(
                [
                    result.patrimonio,              # ex: UF060553
                    result.itemId,                  # ex: 3758
                    result.tipoItem,                # ex: Computer
                    result.action.upper(),          # ex: INSERIR
                    statusAntes,                    # ex: Ativo
                    statusDepois,                   # ex: Em estoque
                    localAntes,                     # ex: CNS
                    localDepois,                    # ex: Devel
...
def associarItemAoChamado(apiUrl: str, headers: Dict[str, str], instruction: Instruction) -> str:
    """
    Interoperabilidade: Estabelece o vínculo relacional entre o hardware (Item) e a requisição de serviço (Ticket) na base de dados Item_Ticket.
    """
...
def removerItemDoChamado(apiUrl: str, headers: Dict[str, str], instruction: Instruction) -> str:
    """
    Manutenção de Integridade: Localiza e remove vínculos existentes entre ativos e chamados, garantindo a correção de associações de inventário.
    """
    ...
    resp = httpx.get(urlList, headers = headers, verify = False, timeout = 10.0)
    if resp.status_code not in (200, 206): return f"List ERRO {resp.status_code}: {resp.text}"

    data = resp.json() or []

    # FILTRO: Vínculos específicos para evitar remoções em cascata indesejadas
    vinculos = [
        r
        for r in data
...
def executeFromParsedTask(sessionToken: str, appToken: str, apiUrl: str, taskInstructions: List[Instruction]) -> tuple[List[Result], Optional[str]]:
    """
    Orquestrador de Fluxo: Coordena a execução sequencial das instruções processadas, gerenciando a coleta de resultados e geração de evidências.
    """
    results: List[Result] = []
    ...
    for i, instruction in enumerate(taskInstructions, 1):
        # Processamento atômico de cada ativo para garantir resiliência do lote
    ...
    if results:
        csvFile = gerarHistoricoCsv(results, results[0].chamadoId, results[0].tarefaId)
        return results, csvFile

    return results, None


def processSingleAsset(apiUrl: str, headers: Dict[str, str], instruction: Instruction) -> Result:
    """
    Motor de Transação de Ativo: Executa a lógica de negócio para atualização de status, localização e vínculos de hardware no GLPI, com tratamento de exceções.
    """
    # Validação de Segurança: Bloqueia a execução se o item não estiver previamente cadastrado
    ...
    # MAPEAMENTO: Dicionário de Mapeamento de Domínio
    mapStatus = {
        "ativo": 7,
        "desfeito": 13,
        "irrecuperável": 1,
    ...
    # IMPORTATE: Construção do Payload Dinâmico para atualização parcial (PATCH/PUT), pois o GLPI exige o ID do item
    fields: Dict[str, Any] = {}
    fields["id"] = itemId 
    ...
    # FUZZYMATCHING: Integração com o Módulo de Localização
    ...
    try:
        # ATUALIZAÇÃO: Muda os critérios físicos (localização/status) no banco de dados via API REST
        if len(fields) > 1: 
            itemUrl = f"{apiUrl.rstrip('/')}/{itemType}/{itemId}"
            rUpdate = httpx.put(itemUrl, headers = headers, json = {"input": fields}, verify = False, timeout = 10.0)
            msgUpdate = f"Upd:{rUpdate.status_code}"

        # ATUALIZAÇÃO: Associa / remove item do chamado
        if action == "inserir":
        ...
        # AUDITORIA: Captura estado final do status e localização
        ...
        lancStatus = (
            f"STATUS_ANTES:{statusAntes}|STATUS_DEPOIS:{statusDepois}|"
            f"LOCAL_ANTES:{localAntes}|LOCAL_DEPOIS:{localDepois}|"
            f"UPDATE:{msgUpdate}|ASSOC:{msgAssoc}"
        )
        ...
        # Tratamento de Falhas e Rollback Lógico de Logs
        lancStatus = (
            f"STATUS_ANTES:{statusAntes}|STATUS_DEPOIS:ERRO|"
            f"LOCAL_ANTES:{localAntes}|LOCAL_DEPOIS:{novaLocal or localAntes}|"
            f"UPDATE:ERRO|ASSOC:ERRO"
        )
        return Result(
            success = False,
            patrimonio = patrimonio,
...
#====================================================================================================================================================================================================
#TASK_CLOSER - from src.logic.task_closer
#====================================================================================================================================================================================================

class GlpiSender:
    """
    Classe Utilitária de Comunicação (Wrapper): Gerencia as requisições de escrita na API do GLPI, lidando com diferentes tipos de Content-Type (JSON e Multipart).
    """
    def __init__(self, apiUrl: str, appToken: str, sessionToken: str):
        self.apiUrl = apiUrl.rstrip("/")
        # Configuração de headers para transações de dados estruturados (JSON)
        self.headersJson = {
            "App-Token": appToken,
            "Session-Token": sessionToken,
            "Content-Type": "application/json",
        }
        # Configuração de headers para upload de arquivos (Binary/Multipart)
        self.headersMultipart = {
            "App-Token": appToken,
            "Session-Token": sessionToken,
        }
        # Instanciação do cliente HTTP com persistência de sessão e supressão de SSL
        self.client = httpx.Client(
    ...
    def getTaskAuthorMention(self, taskId: int) -> str:
        """
        Resolução de Identidade e Menção: Busca metadados do autor original da tarefa para gerar uma menção em formato HTML (Deep Link), personalizando o feedback.
        """
        try:
            ...
            # Composição do Nome Completo (Firstname + Realname) para conformidade e clareza
            primeiroNome = userData.get("firstname") or ""
            sobrenome = userData.get("realname") or ""
            nomeCompleto = f"{primeiroNome} {sobrenome}".strip()
            # 3. Gera a tag de menção compatível com o interpretador RichText do GLPI
    ...
    def markTaskDone(self, taskId: int) -> Dict[str, Any]:
        """
        Gerenciamento de Estado de Tarefa: Atualiza o status da tarefa no banco de dados para 'Feito' (state=2), sinalizando a conclusão do processo automatizado.
        """
        try:
            response = self.client.put(f"/TicketTask/{taskId}", json={"input": {"state": 2}})
            response.raise_for_status()
            return {"success": True}
    ...
    def createInfoTask(self, ticketId: int, message: str) -> Dict[str, Any]:
        """
        Notificação Persistente: Cria uma nova entrada de log (Task de Informação) no chamado, contendo o relatório de execução em formato HTML para visualização do técnico humano.
        """
        try:
            payload = {
                "tickets_id": ticketId,
    ...
    def uploadDocument(self, itemType: str, itemsId: int, filePath: str) -> Dict[str, Any]:
        """
        Persistência de Evidências (Upload de Documentos): Realiza o envio do arquivo de auditoria (CSV) via protocolo Multipart/form-data, vinculando-o à tarefa de conclusão.
        Este módulo garante a integridade documental de todas as ações de inventário.
        """
        if not filePath or not os.path.exists(filePath): return {"success": False, "error": "Arquivo CSV não encontrado."}

        fileName = os.path.basename(filePath)
        # Construção do manifesto de upload conforme requisitos da API REST do GLPI
        manifest = {
            "input": {
        ...
        try:
            with open(filePath, "rb") as f:
                # Payload híbrido: Manifesto (JSON) + Arquivo Binário (CSV)
                files = {
...
def closeTask(apiUrl: str, appToken: str, sessionToken: str, taskId: str, ticketId: str, resultados: List[Any], csvFile: Optional[str] = None) -> Dict[str, Any]:
    """
    Orquestrador de Finalização (Workflow Closer): Coordena o fluxo de encerramento, desde a menção do autor até a geração do relatório HTML e anexo de evidências.
    """
    if not taskId.isdigit() or not ticketId.isdigit(): return {"success": False, "reason": "taskId ou ticketId inválido"}
    ...
    if not taskDone["success"]: return taskDone

    # 3) Processamento e Tabulação de Resultados para o Relatório Final
    sucessos = 0
    total = len(resultados)
    linhasItens = []
    
    for r in resultados:
        # Extração dinâmica de atributos do objeto Result ou dicionário
        ...
        if isSuccess: sucessos += 1
            
        # Define se foi inserido ou removido
        acaoPassado = "removido" if acao == "remover" else "inserido" if acao == "inserir" else acao
            
        # Parsing do pipeline de estados para feedback textual no relatório
        status = "?"
        localizacao = "?"
        for parte in lanc.split("|"):
            if parte.startswith("STATUS_DEPOIS:"): status = parte.split(":", 1)[1]
            elif parte.startswith("LOCAL_DEPOIS:"): localizacao = parte.split(":", 1)[1]
        
        # Geração de linha de log formatada em HTML
        if isSuccess:
            linhasItens.append(f"✅ {pat} foi {acaoPassado}, status atual: {status} e localização: {localizacao}")
        else:
            linhasItens.append(f"❌ {pat} não foi {acaoPassado}. Motivo: {erro or 'Falha sistêmica'}")

    # 4. Composição Semântica da Mensagem Final (Human-Readable Report)
    ...
    # 5) Registro da Task de Informação e Auditoria no Chamado
    ...
    if csvFile and newTaskId:
        docUpload = sender.uploadDocument(itemType = "TicketTask", itemsId = int(newTaskId), filePath = csvFile)
